...
public class Fiber
{
    ...
    public Fiber(Cable cable)
    {
        _cable = cable;
        Id = _cable.Count();
        _cable.Join(this);
        Thread.Sleep(1000);
        Logger.LogInformation($"Run as {_cable.Sender.GetType().Name}, Master : {Cable.Master}, Fiber : {new Location(_cable.Sender.LocalEndPoint, Id)}");
    }

    public async Task SendAsync(Packet packet)
    {
        await _cable.SendAsync(packet);
    }
    ...
    public async Task SendMessageAsync(string endpoint, byte[] content)
    {
        ...
        {
            Source = new Location(_cable.Sender.LocalEndPoint, Id),
    ...
    public async Task<Packet> Request(Packet packet)
    {
        ...
        packet.Source = new Location(_cable.Sender.LocalEndPoint, Id);
    ...
    public async Task OnReceived(Packet packet)
    {
        switch (packet.Proto)
        {
            ...
            {
                await _cable.SendAsync(await OnRequest(packet));
                break;
            }
